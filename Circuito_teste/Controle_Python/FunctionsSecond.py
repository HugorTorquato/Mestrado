...
def Verify_Phases(Phases):
    ...
    if len(Phases) > 3 or len(Phases) == 0:
        print " Phases incorretas1"
        return 0
    elif len(Phases) == 1:
        if Phases == 'A' or Phases == 'B' or Phases == 'C':
            return 1
        else:
            print " Phases incorretas2"
            return 0
    elif len(Phases) == 2:
        if Phases == "AB" or Phases == "BA":
            ...
            print " Phases incorretas3"
            return 0
    elif len(Phases) == 3:
        if Phases == "ABC" or Phases == "BCA" or Phases == "CBA" or Phases == "BAC" or Phases == "ACB"\
                or Phases == "CAB":
            return 1
        else:
            print " Phases incorretas4"
            return 0
    else:
        print " Phases incorretas5"
        return 0
...
def Verifica_Fase(Tensao_Fase):
    ...
    for Fase in Tensao_Fase:
        ...
        if Fase >= DB_Min and Fase <= DB_Max:
            ...
            print "ERRO MEDIÇÃO FASE" + str(Fase)
            break
    return Vector_Y
...
def Leitura_kvar_GDs():
    arquivo = open('C:\Users\hugo1\Desktop\Projeto_Final_Microrredes_Copia_Copia\Circuito_teste\Circuito_OpenDSS\Pot_Geradores\Pot_Reativa.txt', 'r')
    Pot_Reativa = []
...
def Leitura_kW_GDs():
    arquivo = open('C:\Users\hugo1\Desktop\Projeto_Final_Microrredes_Copia_Copia\Circuito_teste\Circuito_OpenDSS\Pot_Geradores\Pot_Ativa.txt', 'r')
    Pot_Ativa = []
    for linha in arquivo:
...
def Leitura_pmult(Nome):
    ...
    arquivo = open('C:\Users\hugo1\Desktop\Projeto_Final_Microrredes_Copia_Copia\Circuito_teste\Circuito_OpenDSS\LoadShapes\Geradores' + str(Nome_GD) + '.txt', 'r')
    pmult = []
    for linha in arquivo:
...
def Leitura_pmult_reduzido():

    Barras = [1, 3, 4, 5, 6, 8, 9, 10, 11, 13, 14, 15, 16, 17, 18, 19, 21, 23, 25, 26, 29, 31]
    fase = ['A', 'B', 'C']
    for k in Barras:
        for f in fase:
            Nome_arquivo = "C:\Users\hugo1\Desktop\Projeto_Final_Microrredes_Copia_Copia\Circuito_teste\Circuito_OpenDSS\LoadShapes\Load_" + str(k) + "_Q_" + str(f) + ".txt"
            arquivo = open(Nome_arquivo, 'r')
            pmult = []
            for linha in arquivo:
                pmult.append(linha.rstrip())
            ...
            pmultCorrigido = []
            step = 30
            for i in range(0, len(pmult), step):
                pmultCorrigido.append(pmult[i])

            arquivo = open(Nome_arquivo, 'w')
            for mult in pmultCorrigido:
                arquivo.write((mult) + "\n")
            arquivo.close()

def Leitura_pmult_reduzido_gdS():

    fase = [4, 5, 6]
    for f in fase:
        Nome_arquivo = "C:\Users\hugo1\Desktop\Projeto_Final_Microrredes_Copia_Copia\Circuito_teste\Circuito_OpenDSS\LoadShapes\Geradores\Q_GD_" + str(f) + ".txt"
        arquivo = open(Nome_arquivo, 'r')
        pmult = []
        for linha in arquivo:
            pmult.append(linha.rstrip())
...
def Escreve_pmult(Nome, pmult):
    ...
    Nome_GD = ("\P" + str(Nome.upper()))
    arquivo = open('C:\Users\hugo1\Desktop\Projeto_Final_Microrredes_Copia_Copia\Circuito_teste\Circuito_OpenDSS\LoadShapes\Geradores' + str(Nome_GD) + '.txt', 'w')
    for mult in pmult:
        arquivo.write((mult) + "\n")
    arquivo.close()
...
def Salva_Arquivo(self, Var, nome):
        ...
        for GD in self.dssGenerators.AllNames:
            ...
            arquivo = open('C:\Users\hugo1\Desktop\Projeto_Final_Microrredes_Copia_Copia\Testes\Sistema_Menor\Testes\_' + str(
...
def Armazena_Dados_Tensao(Tensao):

    Bus_Voltage_Daily = ([])
    for Barra in range(26):  # Número de barras
        Bus_Voltage_Daily.append([])
        for Bus_Voltage in Tensao:
            Bus_Voltage_Daily[Barra].append(Bus_Voltage[Barra])

    i = 0
    for Barra in Bus_Voltage_Daily:  # 26 barras
        arquivo = open('C:\Users\hugo1\Desktop\Projeto_Final_Microrredes_Copia_Copia\Circuito_teste\Controle_Python\Resultados\Tensao_Barras\_Barra'
                       + str(i) + '.txt', 'w')
        for Row in Barra:  # 1440 iterações
            arquivo.write((str(Row)) + "\n")
        arquivo.close()
        i += 1

def Armazena_Dados_Desq(Desq):

    Bus_Desq_Daily = ([])
    for Barra in range(26):  # Número de barras
        Bus_Desq_Daily.append([])
        for Bus_Desq in Desq:
            Bus_Desq_Daily[Barra].append(Bus_Desq[Barra])

    i = 0
    for Barra in Bus_Desq_Daily:  # 26 barras
        arquivo = open('C:\Users\hugo1\Desktop\Projeto_Final_Microrredes_Copia_Copia\Circuito_teste\Controle_Python\Resultados\Desq_Barras\_Barra'
                       + str(i) + '.txt', 'w')
        for Row in Barra:  # 1440 iterações
            arquivo.write((str(Row)) + "\n")
        arquivo.close()
